...
public class MjMeshFilter : MonoBehaviour {
  ...
  protected void Update() {
    ...
    // (spar patch) GetInstanceID() is a hard-deprecation error on Unity 6.5;
    // the id was only a debugging aid, so name the mesh by owner alone.
    mesh.name = $"Mujoco mesh for {gameObject.name}";
